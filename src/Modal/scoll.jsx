...
function SignUp() {
  const cards = [
    { id: 1, title: "React", desc: "JavaScript library for UI" },
    { id: 2, title: "Vue", desc: "Progressive JS Framework" },
    { id: 3, title: "Angular", desc: "Full-featured framework" },
    { id: 4, title: "Svelte", desc: "Compiles to tiny, fast JS" },
    { id: 5, title: "Next.js", desc: "Production React framework" },
    { id: 6, title: "Remix", desc: "Web fundamentals + React" },
     { id: 4, title: "Svelte", desc: "Compiles to tiny, fast JS" },
    { id: 5, title: "Next.js", desc: "Production React framework" },
    { id: 6, title: "Remix", desc: "Web fundamentals + React" },
  ];
    ...
    <div className="w-full px-4 py-6">
      <div className="flex gap-4 overflow-x-auto scroll-smooth">
        {cards.map((card) => (
          <div
            key={card.id}
            className="min-w-[16rem] flex-shrink-0 bg-white border border-gray-300 rounded-lg p-4 shadow-md hover:shadow-lg transition-all duration-300"
          >
...
function DashBoard() {
  ...
  logins.map((entry, i) => (
    <div key={i} className="border p-2 my-2">
      ...
      <p>🖥️ {entry.device} | {entry.browser} | {entry.os}</p>
      <p>🕒 {entry.time}</p>
      <hr />
    </div>
  ));
  ...
  return (
    ...
    <div
      className=" bg-black flex bg-gradient-to-br from-black via-zinc-900"


    >
      <aside
        className="min-h-screen w-[280px] flex flex-col p-6 text-white backdrop-blur-md shadow-lg"
        style={{ fontFamily: "obitron" }}
      >
        <h1
          className="text-4xl font-extrabold px-6 py-4 rounded-xl shadow-lg"
          style={{
        ...
        >
          <button
            ...
            className={`flex items-center gap-3 px-2 py-2  text-white  transition-all  rounded ${selectedSection === "dashboard"
          ...
          <button
            ...
            className={`flex items-center gap-3 px-2 py-2  text-red-600  rounded transition-all  ${selectedSection === "Profile"
              ? "bg-slate-300 text-black shadow-md"
              : "text-white hover:bg-slate-700"
          ...
          <button
            ...
            className={`flex items-center gap-3 px-2 py-2  text-white  rounded transition-all ${selectedSection === "Drivers"
              ? "bg-slate-300 text-black shadow-md"
              : "text-white hover:bg-slate-700"
              }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-3 px-2 py-2 text-white rounded transition-all ${selectedSection === "Teams"
              ? "bg-slate-300 text-black shadow-md"
              : "text-white hover:bg-slate-700"
              }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-3 px-2 py-2  text-white rounded transition-all ${selectedSection === "Analytics"
              ? "bg-slate-300 text-black shadow-md"
              : "text-white hover:bg-slate-700"
              }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-3 py-2 px-2  text-white rounded transition-all ${selectedSection === "users"
              ? "bg-slate-300 text-black shadow-md"
              : "text-white hover:bg-slate-700"
              }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-3 py-2  text-white rounded transition-all ${selectedSection === "Login History"
              ? "bg-slate-300 text-black shadow-md"
              : "text-whte hover:bg-slate-700"
        ...
        >
          <button
            className="flex items-center gap-3 cursor-pointer px-4 py-2 text-white hover:bg-slate-700 rounded transition-all text-sm"
      ...
      <main className="flex-1 p-10 overflow-hidden">
        <header className="flex justify-between items-center mt-8 mb-10 text-white">
          <h1
          ...
          <button className="relative  text-white p-3   hover:bg-slate-700 shadow-lg transition-transform hover:scale-110">
            <FaBell size={20} />
            <span className="absolute -top-1 -right-1 rounded-full w-4 flex items-center justify-center text-sm  font-bold animate-pulse  bg-white h-4"></span>
          </button>
        </header>
        ...
        {selectedSection === "Login History" && (
          <>
            <section >
              <h2 className="text-2xl text-white font-bold mb-4 pb-2">Login History</h2>
              <div className="space-y-4">
                {(JSON.parse(localStorage.getItem("LoginHistory")) || []).map((entry, index) => {
                  return (
                    <div
                      key={index}
                      className="border border-white/20 p-4 rounded-xl bg-white/10 backdrop-blur-sm"
                    >
                      <p className="text-white">📍 <strong>IP:</strong> {entry.ip}</p>
                      <p className="text-white">📍 <strong>City:</strong> {entry.city}, {entry.country}</p>
                      <p className="text-white">🖥️ <strong>Device:</strong> {entry.device}</p>
                      <p className="text-white">🌐 <strong>Browser:</strong> {entry.browser}</p>
                      <p className="text-white">💻 <strong>OS:</strong> {entry.os}</p>
                      <p className="text-white">🕒 <strong>Time:</strong> {entry.time}</p>
                    </div>
